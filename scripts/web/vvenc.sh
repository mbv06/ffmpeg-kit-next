...
# WEB (WASM) VVENC BUILD
# CMake build via Emscripten. vvenc is C++; runtime resolves at the main-module
# link, like x265. x86 SIMD (SIMDE on other targets) is disabled for a portable-C++
# build. EMSCRIPTEN_SYSTEM_PROCESSOR overrides the toolchain's default x86
# processor report (see the x265 script). Static + PIC.
