...
# WEB (WASM) LIBAOM BUILD
# CMake build via Emscripten (aom's build system supports Emscripten upstream).
# AOM_TARGET_CPU=generic selects the portable-C code paths and skips run-time cpu
# detection, so the wasm32 processor value never reaches aom's arch dispatch.
# Static + PIC.
