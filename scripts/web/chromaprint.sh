...
# WEB (WASM) CHROMAPRINT BUILD
# CMake build via Emscripten. chromaprint is C++; its C++ runtime resolves at the
# main-module link, like x265. The bundled kissfft backend is used (no external
# FFT dependency). Static + PIC.
