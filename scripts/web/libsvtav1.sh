...
# WEB (WASM) SVT-AV1 BUILD
# CMake build via Emscripten. COMPILE_C_ONLY selects the portable-C code paths
# (no x86/arm assembly or intrinsics). SVT-AV1 warns that 32-bit targets are
# unsupported but the C-only build compiles for wasm32. Static + PIC.
