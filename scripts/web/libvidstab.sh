...
# WEB (WASM) LIBVIDSTAB BUILD (GPL)
# CMake build via Emscripten. x86 SSE paths are disabled explicitly (the wasm
# host has no SSE); OpenMP is unavailable under Emscripten. Requires FFmpeg's
# --enable-gpl to actually be linked. Static + PIC.
...
# vid.stab declares cmake_minimum_required(2.x), which modern CMake refuses
