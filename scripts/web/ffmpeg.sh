...
# SET CONFIGURE OPTIONS FOR ENABLED EXTERNAL LIBRARIES
# Mirrors the Android build (web is a from-source cross-compile). Only libraries
# that were enabled and successfully built reach here; FFmpeg is told to link them
# via pkg-config. Web's configure passes --disable-autodetect, so unlisted
# libraries generally do not need explicit --disable flags; system-provided
# libraries handled in this loop may still need one.
for library in {0..61} ${LIBRARY_VVENC} ${LIBRARY_LIBSVTAV1} ${LIBRARY_LIBJXL} ${LIBRARY_LIBLC3} ${LIBRARY_WEB_LIBICONV}; do
  if [[ ${ENABLED_LIBRARIES[$library]} -eq 1 ]]; then
    ...
    freetype)
      ...
      # freetype (and libpng below it) use Emscripten's zlib port; the flag makes
      # -lz resolvable in configure's executable link tests. Side-module links
      # never see -lz: it is stripped from EXTRALIBS below and the main module
      # links the port instead.
      CFLAGS+=" -sUSE_ZLIB=1"
      LDFLAGS+=" -sUSE_ZLIB=1"
    ...
    libvorbis)
      ...
      ;;
    libvpx)
      # Web links FFmpeg as SIDE_MODULE shared libs, where emscripten whole-includes
      # static archives. FFmpeg's --enable-libvpx already adds -lvpx, so adding it
      # again here double-includes libvpx.a (duplicate symbols). Provide only the
      # header path and the -L search path; let FFmpeg supply the single -lvpx.
      ...
      ;;
    libwebp)
      # Only header path + -L search path; FFmpeg's --enable-libwebp supplies -lwebp
      # (see libvpx note). Adding it here too would duplicate-include the archive.
    ...
    vo-amrwbenc)
      ...
      ;;
    x264)
      # header path + -L only; FFmpeg's --enable-libx264 supplies -lx264 (see libvpx).
      ...
      ;;
    x265)
      # header path + -L only; FFmpeg's --enable-libx265 supplies -lx265 (see libvpx).
