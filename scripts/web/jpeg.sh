...
# WEB (WASM) JPEG (libjpeg-turbo) BUILD
# CMake build via Emscripten. SIMD is x86/arm assembly only, so it is disabled for
# wasm (portable C). Static + PIC (CFLAGS carry -fPIC). run-web.sh pre-cleaned
# ${BUILD_DIR} and exported the toolchain/flags.
