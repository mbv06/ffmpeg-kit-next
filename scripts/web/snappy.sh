...
# WEB (WASM) SNAPPY BUILD
# CMake build via Emscripten. snappy is C++; its C++ runtime resolves at the
# main-module link, like x265. zlib is disabled (HAVE_LIBZ=0): the core compressor
# does not use it, only the removed tests/benchmarks did. Static + PIC.
