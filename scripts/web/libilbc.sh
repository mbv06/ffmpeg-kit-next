...
# WEB (WASM) LIBILBC BUILD
# CMake build via Emscripten. libilbc vendors abseil as a git submodule; the
# arthenica mirror rewrite (matching the other platforms) points it at a fork that
# is guaranteed available. C++ runtime resolves at the main-module link, like
# x265. Static + PIC.
